...
class CategoriasNoticiaController extends Controller
{
	/**
	...
	{
		$model=new CategoriasNoticia;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['CategoriasNoticia']))
		{
			$model->attributes=$_POST['CategoriasNoticia'];
			if($model->save()){
	...
	{
		/*$dataProvider=new CActiveDataProvider('CategoriasNoticia');
        ...
        if (isset($_REQUEST['sSearch']) && isset($_REQUEST['sSearch']{0})) {
            ...
            Yii::app()->session['categoriasNoticiaSearch'] = $_REQUEST['sSearch'];
        }
...
//        $pagination->setPageSize(10);
        ...
        $dataProvider = new CActiveDataProvider('CategoriasNoticia', array(
            'criteria' => $criteria,
        ...
        $widget = $this->createWidget('ext.nineinchnick.edatatables.EDataTables', array(
            ...
            'dataProvider' => $dataProvider,
            'ajaxUrl' => $this->createUrl('/categoriasNoticia/index?a'),
            'columns' => $columns,
...
//                'fnDrawCallbackCustom' => "js:function(){\$('a[rel=tooltip]').tooltip(); \$('a[rel=popover]').popover();}",
	 ...
	 * @return CategoriasNoticia the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=CategoriasNoticia::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param CategoriasNoticia $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='categorias-noticia-form')
		{

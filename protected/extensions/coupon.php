...
class coupon {
     ...
     * @throws Exception
     */
    static public function generate($options = array()) {

        $length         = (isset($options['length']) ? filter_var($options['length'], FILTER_VALIDATE_INT,array('options' => array('default' => self::MIN_LENGTH, 'min_range' => 1))) : self::MIN_LENGTH );
        ...
        $useSymbols     = false;
        $useMixedCase   = false;
        $mask           = (isset($options[0]) ? filter_var($options[0], FILTER_SANITIZE_STRING) : false );*/
     ...
     * @return array
     */
    static public function generate_coupons($maxNumberOfCoupons = 1, $options = array()) {
        $coupons = array();
     ...
     * @param array $options
     */
    static public function generate_coupons_to_xls($maxNumberOfCoupons = 1, $filename, $options = array()) {
        $filename = (!trim($filename) ? 'coupons' : trim($filename));
